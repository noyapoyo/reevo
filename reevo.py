...
class ReEvo:
    ...
    def evaluate_population(self, population: list[dict]) -> list[float]:
        """
        Evaluate population one candidate at a time.

        The evaluator writes each candidate to problems/<problem>/gpt.py before
        running eval.py, so parallel execution can make one candidate evaluate a
        different candidate's code.
        """
        for response_id in range(len(population)):
            if self.function_evals >= self.cfg.max_fe:
                population[response_id] = self.mark_invalid_individual(
                    population[response_id],
                    f"Evaluation budget exhausted: max_fe={self.cfg.max_fe}",
                )
                continue
            ...
            try:
                inner_run = self._run_code(population[response_id], response_id)
                try:
